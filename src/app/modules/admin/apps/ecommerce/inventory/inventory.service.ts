...
export class InventoryService
{
    ...
    private _policy: BehaviorSubject<Policy | null> = new BehaviorSubject(null);
     ...
     * @param search
     */
    getPolicies(page: number = 0, size: number = 10, sort: string = 'name', order: 'asc' | 'desc' | '' = 'asc', search: string = ''):
        Observable<{ pagination: InventoryPagination; policies: Policy[] }>
    {
        return this._httpClient.get<{ pagination: InventoryPagination; policies: Policy[] }>('api/policies', {
            params: {
                page: '' + page,
                size: '' + size,
                sort,
            ...
            {
                this._pagination.next(response.pagination);
                this._policies.next(response.policies);
            }),
        );
    }
    ...
    getPolicyById(id: string): Observable<Policy>
    {
        return this._policies.pipe(
            ...
            switchMap((policy) =>
            {
                if ( !policy )
                {
                    return throwError('Could not found policy with id of ' + id + '!');
                }
    ...
    createPolicy(policy: Policy): Observable<Policy>
    {
        return this.policies$.pipe(
            take(1),
            switchMap(policies => this._httpClient.post<Policy>('api/apps/ecommerce/inventory/policy', policy).pipe(
                map((newPolicy) =>
                {
                    // Update the products with the new product
                    this._policies.next([newPolicy, ...policies]);
